...
export default function App() {
  ...
  const renderField = (field: any, index: number) => {
    const inputClasses = "w-full px-4 py-2 bg-[#F3F4F6] border-none text-slate-800 placeholder-slate-400 rounded-[14px] focus:outline-none focus:ring-2 focus:ring-[#A2EBF2]/50 text-sm transition-all";
    ...
    if (field.input.type === "text" || field.input.type === "tel" || field.input.type === "number" || field.input.type === "email" || field.input.type === "password" || field.input.type === "url") {
      ...
      return (
        <div className="flex flex-row gap-2.5 items-center px-1 py-0.5">
          <input type={field.input.type} id={field.input.id} name={field.input.name} checked={isChecked} onChange={() => { }} disabled={field.input.disabled} className="rounded-md border-slate-300 bg-white text-cyan-600 focus:ring-cyan-200 h-4 w-4" />
          <span className="text-slate-700 text-xs font-bold max-w-[200px] truncate">{field.label.text}</span>
        </div>
      );
    }
    return (
  ...
  // VISTA PRINCIPAL
  return (
    <div className="w-screen min-h-screen bg-[#F4F6F9] text-slate-800 p-5 font-sans pb-10">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-1.5">
          <button
            ...
            title="Configuración"
            className="w-9 h-9 bg-white border border-slate-100/80 shadow-sm hover:shadow-md text-slate-500 hover:text-slate-800 rounded-full flex items-center justify-center transition-all hover:scale-105 active:scale-95 group relative"
          >
            {/* Pequeño punto indicador si RunInBackground está activo */}
            {runInBackground && <span className="absolute top-1 right-1 w-2 h-2 bg-amber-500 rounded-full animate-pulse shadow-md border border-white"></span>}
          ...
          <button
            ...
            className="w-9 h-9 bg-white border border-slate-100/80 shadow-sm hover:border-red-100 hover:bg-red-50 text-slate-400 hover:text-red-500 rounded-full flex items-center justify-center transition-all hover:scale-105 active:scale-95 disabled:opacity-40"
          >
      ...
      <div className="flex justify-between items-center bg-white border border-slate-100/80 px-4 py-4 rounded-3xl mb-4 shadow-[0_8px_25px_rgba(0,0,0,0.04)] border-transparent">
        <div className="flex items-center gap-2">
          <div className="w-5 h-5 bg-[#A2EBF2]/20 rounded-full flex items-center justify-center text-cyan-600">
            <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 7v10c0 2.21 3.582 4 8 4s8-1.79 8-4V7M4 7c0 2.21 3.582 4 8 4s8-1.79 8-4M4 7c0-2.21 3.582-4 8-4s8 1.79 8-4m0 5c0 2.21-3.582 4-8 4s-8-1.79-8-4" />
            </svg>
          </div>
          <span className="font-bold text-slate-800 text-sm">Biblioteca de Datos</span>
        </div>
        <span className="px-2.5 py-1 bg-[#DDF6E3] text-[#22823B] text-xs rounded-full border border-[#BFF0CA] font-bold flex items-center gap-1.5">
          <span className="w-1.5 h-1.5 rounded-full bg-[#22823B] animate-pulse" />
          {savedCount} registros
        </span>
      </div>

      <div className="bg-white border border-slate-100/80 rounded-3xl overflow-hidden shadow-[0_8px_25px_rgba(0,0,0,0.04)] border-transparent">
        <button
          onClick={() => setIsOpen(!isOpen)}
          className="w-full px-5 py-3.5 flex items-center justify-between hover:bg-slate-50/50 transition-colors duration-200 border-b border-slate-50"
        >
          <div className="flex items-center gap-2">
            <span className="font-bold text-slate-800 text-sm">Escaneados en Pantalla</span>
        ...
        <div className={`transition-all duration-300 ease-in-out ${isOpen ? 'max-h-[300px] overflow-y-auto p-4 flex flex-col gap-2' : 'max-h-0 overflow-hidden'}`}>
          {fields.map((field, index) => (
            <div key={index} className="flex flex-col gap-1 bg-[#FAFBFC] rounded-2xl border border-slate-50/80">
      ...
      <div className="grid grid-cols-2 gap-3 mt-4">
        <button
          onClick={() => loadFields()}
          disabled={loading}
          className="px-4 py-3 bg-white hover:bg-slate-50 text-slate-700 font-bold rounded-2xl border border-slate-100/80 shadow-[0_4px_15px_rgba(0,0,0,0.04)] hover:scale-[1.02] active:scale-[0.98] transition-all duration-200 flex items-center justify-center gap-2 disabled:opacity-50"
        >
        ...
        <button
          ...
          disabled={loading}
          className="px-4 py-3 bg-slate-900 hover:bg-slate-800 text-white font-bold rounded-2xl shadow-[0_4px_15px_rgba(0,0,0,0.06)] hover:scale-[1.02] active:scale-[0.98] transition-all duration-200 flex items-center justify-center gap-2 disabled:opacity-50"
        >
      ...
      <div className="grid grid-cols-1 gap-2 mt-4">
        <label className={`flex items-center justify-between p-4 border border-slate-100/80 rounded-2xl cursor-pointer transition-all duration-200 bg-white hover:bg-slate-50/40 shadow-[0_6px_20px_rgba(0,0,0,0.03)]`}>
          <div className="flex flex-col">
            <span className="text-sm font-bold text-slate-800">Guardar Automáticamente</span>
            <span className="text-xs text-slate-400 mt-0.5">Recopila datos cada 5 segundos</span>
          </div>
          <div className={`w-10 h-6 rounded-full p-1 transition-colors duration-300 ease-in-out flex flex-shrink-0 items-center ${autoSave ? 'bg-emerald-500' : 'bg-slate-200'}`}>
        ...
        <label className={`flex items-center justify-between p-4 border border-slate-100/80 rounded-2xl cursor-pointer transition-all duration-200 bg-white hover:bg-slate-50/40 shadow-[0_6px_20px_rgba(0,0,0,0.03)]`}>
          <div className="flex flex-col">
            <span className="text-sm font-bold text-slate-800">Cargar Automáticamente</span>
            <span className="text-xs text-slate-400 mt-0.5">Al cambiar de página o navegar</span>
          </div>
          <div className={`w-10 h-6 rounded-full p-1 transition-colors duration-300 ease-in-out flex flex-shrink-0 items-center ${autoLoad ? 'bg-blue-500' : 'bg-slate-200'}`}>
      ...
      {(autoSave || autoLoad) && (
        <div className="flex items-start gap-2 mt-3 p-3 bg-white border border-slate-100 rounded-2xl shadow-[0_4px_15px_rgba(0,0,0,0.02)]">
          <svg className="w-4 h-4 text-amber-500 flex-shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
          <div className="flex flex-col">
            <span className="text-xs font-bold text-slate-800">Tip de Uso</span>
